...
while larger models introduce more unstable activation states that exponentially expand 
...
the number of required subproblems scales with $2^{n_x}$. 
...
Consequently, the solver cannot find valid control solutions in certain regions of the state space, 
leaving the policy effectively without expert supervision in these infeasible subdomains.
...
incorporating four intermediate forward passes into the computational graph would amplify relaxation errors in LiRPA.
...
the sim-to-real gap must be addressed by including bounded parameter uncertainties and disturbance sets directly 
