...
on intermediate neurons, yielding loose bounds and unnecessary branching~\cite{xuFastCompleteEnabling2021,wang2021beta}.
...
The combination of the split constraints with the relaxation slopes $\alpha \in [0, 1]^K$ from $\alpha$-CROWN 
...
\begin{equation}
    ...
    \underline{\psi}^*_{\alpha,\beta} = \max_{\alpha \in [0, 1]^K, \, \beta \ge 0} \mathcal{G}(\alpha, \beta) \;\le\; \psi^\star_{\mathrm{sub}}.
\end{equation}
...
Because any feasible configuration $\left( \alpha \in [0, 1]^K, \beta \ge 0 \right)$ corresponds to a valid dual relaxation, 
