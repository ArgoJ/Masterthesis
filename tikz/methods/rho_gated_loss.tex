\begin{tikzpicture}[
    ...
    \node[mathloss] (w_rho) at (-5.0, 0 |- Y_lcond) {
        ...
        $\displaystyle w_{\rho} = \sigma \left( s \cdot \frac{\rho - V_\phi(x)}{\max (\rho, \varepsilon_{\text{rel}})} \right)$
    };

    % 3.2 Relative Decrease Loss (L_dec)
    \node[mathloss] (l_dec) at (-0.5, 0 |- Y_loss) {
        \textbf{Rel. Decrease Loss} $\mathcal{L}_{\text{dec}}$\\[4pt]
        $\displaystyle = \left[ \frac{V_\phi(x^+_V) - (1 - \kappa) V_\phi(x)}{\max(|V_\phi(x)|, \varepsilon_{\text{rel}})} \right]_+$
    };
